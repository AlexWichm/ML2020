...
def gaussianRbf(centerX, centerY, sigmaSquare, pointX, pointY):
  return np.exp(-(((centerX-pointX)**2+(centerY-pointY)**2)/(2*sigmaSquare)))

def extractFeatures(xy):
  x = xy[0]
  y = xy[1]
  #return [x, y**2*np.sqrt(np.abs(y))] #an almost good separation by chance
  #return [x * x, y * x] # good separation
  #standard deviation is 1 and center is 0,0 so this should contain 99.7% of all points
  #-> all points since we have 300
  return [x, gaussianRbf(0,0,1*9,x,y)]
